...
import com.example.yanglin.ongoingdemo1.Route.DrivingRouteOverlay;
...
public class MainActivity  extends AppCompatActivity implements AMap.OnMapClickListener,
    ...
    LatLonPoint mStartPoint ;//=new LatLonPoint(108.925302, 34.233669) ; //起点，116.335891,39.942295  几何点对象类  = new LatLonPoint(39.942295, 116.335891)
     LatLonPoint mEndPoint;//= new LatLonPoint(108.953196, 34.229055)  ;//终点，116.481288,39.995576   = new LatLonPoint(39.995576, 116.481288)
    ...
    public void searchRouteResult(int routeType, int mode) {
       ...
       // mStartPoint= GetValueStart();
        ...
        geocoderSearchStart.setOnGeocodeSearchListener(new GeocodeSearch.OnGeocodeSearchListener() {
            ...
            public void onGeocodeSearched(GeocodeResult geocodeResult, int i) {
                GeocodeAddress addr = geocodeResult.getGeocodeAddressList().get(0);
                mStartPoint = addr.getLatLonPoint();
            }
        });
        GeocodeQuery qu = new GeocodeQuery(startStr,"");
        geocoderSearchStart.getFromLocationNameAsyn(qu);
...
// 将终点的地理位置转化为经纬度
        ...
        geocoderSearchEnd.setOnGeocodeSearchListener( new GeocodeSearch.OnGeocodeSearchListener(){
            ...
            public void onGeocodeSearched(GeocodeResult geocodeResult, int i) {
                GeocodeAddress addr = geocodeResult.getGeocodeAddressList().get(0);
                mEndPoint = addr.getLatLonPoint();
            }
        });
        GeocodeQuery qu1 = new GeocodeQuery(endStr,"");
        geocoderSearchEnd.getFromLocationNameAsyn(qu1);


        //mEndPoint = GetValueEnd();
        System.out.println("*******经纬度mStartPoint:"+mStartPoint);
        if (mStartPoint == null) {
 ...
 @Override
    public void onDriveRouteSearched(DriveRouteResult driveRouteResult, int errorCode) {
     ...
     steps = drivePath.getSteps(); //获取路段的个数
               System.out.println("*****返回驾车路段的个数******："+steps.size());
            float distance=0;
               LatLng latLng1 = new LatLng(mStartPoint.getLatitude(),mStartPoint.getLongitude());
                 for (DriveStep step : steps) {
                     System.out.println("一个step长度"+step.getDistance());
                     ...
                     distance = AMapUtils.calculateLineDistance(latLng1,latLng2);
                           System.out.println("此时的距离为："+distance);
                           if(distance>1000){    //比较将来要行驶的1 公里
                               break;
                           }
                           String s1=String.valueOf(lat);
                           String s2=String.valueOf(lng);
                           System.out.println("测试输出："+s1+","+s2);
                            MD5 md = new MD5();
                        String   s = md.print(s1,s2);
